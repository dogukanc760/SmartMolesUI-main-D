...
const IrrigatonTypeDetail = () => {
  ...
  let { irrigationTypeID } = useParams();

  const onDelete = async (irrigationTypeID) => {
    const deleted = services.deleteIrrigationType(irrigationTypeID);
    if ((await deleted).status == 200) {
      handleClose();
      navigate('/dashboard/irrigation-types');
    }
  };
  ...
  const update = async (irrigationTypeID) =>{
    setIrrigationType(dataFinal)
    console.log('first')
    console.log(dataFinal)
    const updated = await services.updateIrrigationType(irrigationTypeID, {
        'TitleEN':dataFinal.TitleEN,
   ...
   useEffect(() => {
     const fetchData = async (irrigationTypeID) => {
       return await services.getByIrrigationTypeId(irrigationTypeID);
     };
     fetchData(irrigationTypeID).then((data) => {
    ...
    <Page title="Dashboard: Bitki Türleri">
      <Container maxWidth="xxl">
            ...
            <Fade in={open}>
              <Box sx={style}>
                ...
                <Stack sx={{ mt: 5 }} direction="row" alignItems="center" justifyContent="space-evenly">
                  <Button sx={{ mr: 2 }} onClick={() => onDelete(irrigationTypeID)} variant="outlined" color="error">
                    Sil
                  </Button>
        ...
        <Card sx={{mt:2}}>
          ...
          >
            <Button color="primary" variant="contained" type="submit" onClick={() =>update(irrigationTypeID)}>
              Güncelle
            </Button>
          </Box>
        </Card>
...
export default IrrigatonTypeDetail;
